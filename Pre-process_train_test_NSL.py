...
# Create X_train containing these columns from df_train, features selected from the 41 of the NSL-KDD set
# (I assume these features, as well as the ones for U2R and R2L are obtained from expertise and literature)
...
with open('NSL-KDD Outputs/Results.txt', 'a') as output_file:
    ...
    output_file.write('\nProbe results correctness: ' + (str(result[probe_index].sum() / result[probe_index].shape[0]*100)) + '%')
